 ...
 */
function updateCellValue(editableGrid, rowIndex, columnIndex, oldValue, newValue, row, id, onResponse)
{      
	$.ajax({
...
{ 
	this.editableGrid = new EditableGrid("agency_requests", {
		enableSort: true,
      ...
      // Once the table is displayed, we update the paginator state
		...
		modelChanged: function(rowIndex, columnIndex, oldValue, newValue, row, id) {
   	    	updateCellValue(this, rowIndex, columnIndex, oldValue, newValue, row, id);
       	}
 	});
	this.fetchGrid(); 
...
DatabaseGrid.prototype.fetchGrid = function()  {
	...
	this.editableGrid.loadXML("loaddata_agn_request.php");
};
...
// render for the action column
	grid.setCellRenderer("action", new CellRenderer({ 
		render: function(cell, id) {                 
		      cell.innerHTML+= "<i onclick=\"datagrid.approveReq("+id+");\" class='fa fa-check' ></i>";
		}
	}));
	grid.setCellRenderer("action2", new CellRenderer({ 
		render: function(cell, id) {                 
...
DatabaseGrid.prototype.approveReq = function(id) 
{
  ...
  if ( confirm('Are you sure you want to approve request ' + id )  ) {
		...
		{ 
			if (response == "ok" )
		        self.editableGrid.removeRow(1000);
		},
		error: function(XMLHttpRequest, textStatus, exception) { alert("Ajax failure\n" + errortext); },
		async: true
	});
